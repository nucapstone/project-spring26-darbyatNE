...
- **Challenge #2: Consistent Separation of Retail Bill Items**: Isolating the "retail energy" cost (e.g., generation/supply charges) from retail bills for direct comparison to wholesale LMPs is possible for many providers via rate schedule parsing, but inconsistency in bundling (e.g., generation mixed with T&D) may require estimation or manual validation. We'll prioritize utilities with clear breakdowns and develop algorithms for approximation where needed.
- **Challenge #3: Identifying Retail Classes for Comparison**: We'll focus on key retail classes for initial comparisons, starting with Residential (e.g., Delmarva's Residential Time-of-Use Rates, with on-peak/off-peak structures) and Commercial (e.g., Small General, Medium General, Large General). These represent the majority of customers and align with LMP volatility. Industrial/Lighting classes (e.g., Delmarva's Lighting) are lower priority due to niche usage but can be added in stretch goals.
