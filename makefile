...
ifeq ($(DETECTED_OS),Darwin)
	...
	@osascript -e 'tell app "Terminal" to do script "cd $(ROOT_DIR) && $(TUNNEL_CMD_OSASCRIPT) && source $$(conda info --base)/etc/profile.d/conda.sh && conda activate $(CONDA_ENV_NAME) && npm run dev; exec bash"'
...
ifneq ($(IS_WSL),)
	...
	$(CHECK_SSH_KEY)
	@$(TUNNEL_CMD)
	@source $$(conda info --base)/etc/profile.d/conda.sh && conda activate $(CONDA_ENV_NAME) && npm run dev
else
	@echo "Checking for existing tunnels..."
	@$(KILL_TUNNEL)
	$(CHECK_AND_FREE_PORTS)
	...
	$(CHECK_SSH_KEY)
	@x-terminal-emulator -e bash -c "cd $(ROOT_DIR) && $(TUNNEL_CMD) && source \$$(conda info --base)/etc/profile.d/conda.sh && conda activate $(CONDA_ENV_NAME) && npm run dev; exec bash" || xterm -e bash -c "cd $(ROOT_DIR) && $(TUNNEL_CMD) && source \$$(conda info --base)/etc/profile.d/conda.sh && conda activate $(CONDA_ENV_NAME) && npm run dev; exec bash"
endif
	...
	@start cmd /k "cd /d $(ROOT_DIR) && start /b $(TUNNEL_CMD) && call conda activate $(CONDA_ENV_NAME) && npm run dev"
