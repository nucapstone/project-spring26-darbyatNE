...
def run_upsert():
    ...
    WITH hourly_territory_avg AS (
        -- Step 1: Calculate the true average price for the territory for EACH HOUR
        SELECT 
            r.service_territory,
            p.datetime_beginning_ept,
            AVG(p.total_lmp_da) AS hourly_avg_price
        FROM pjm_da_hrl_lmps p
        -- Using DISTINCT here guarantees duplicate mappings don't multiply your rows
    ...
    -- Step 2: Roll those clean hourly averages into the monthly INSERT
    SELECT 
        service_territory,
